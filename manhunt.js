    ...
    for (let i = 0; i < poses.length; i++) {
        ...
        for (let j = 0; j < pose.keypoints.length; j++) {
            ...
            if (keypoint == pose.keypoints[0] && keypoint.score > 0.2) {
                push();
                imageMode(CENTER);
                image(dream, keypointX, keypointY, 60, 60); //498
                fill(255, 0, 0);
                noStroke();
...
function Target() {
    ...
    this.display = function() {
        push();
        image(george,this.x,this.y,this.width,this.height);
    ...
    this.collide = function() {
        collided = collideRectRect(this.x, this.y, this.width, this.height, keypointX - 30, keypointY - 30, 60, 60); // minuus 30 to center hitbox
